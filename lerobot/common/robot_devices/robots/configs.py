...
@RobotConfig.register_subclass("fcbox")
@dataclass
class FcboxRobotConfig(RobotConfig):
    # `max_relative_target` limits the magnitude of the relative positional target vector for safety purposes.
    # Set this to a positive scalar to have the same value for all motors, or a list that is the same length as
    # the number of motors in your follower arms.
    max_relative_target: int | None = None
    ...
    cameras: dict[str, CameraConfig] = field(
        default_factory=lambda: {
            "front": OpenCVCameraConfig(
                camera_index="/dev/video1", fps=30, width=640, height=480
            ),
            # "wrist_left": OpenCVCameraConfig(
            #     camera_index="/dev/video2", fps=30, width=640, height=480, rotation=90
            # ),
    ...
    calibration_dir: str = ".cache/calibration/fcbox"

    leader_arms: dict[str, MotorsBusConfig] = field(
        default_factory=lambda: {
            "left": FeetechMotorsBusConfig(
                port="/dev/aloha_leader_arm_left",
                motors={
                    # name: (index, model)
                    "shoulder_pan": [1, "sts3215"],
                    "shoulder_lift": [2, "sts3215"],
            ...
            "right": FeetechMotorsBusConfig(
                port="/dev/aloha_leader_arm_right",
                motors={
                    # name: (index, model)
                    "shoulder_pan": [1, "sts3215"],
                    "shoulder_lift": [2, "sts3215"],
    ...
    follower_arms: dict[str, MotorsBusConfig] = field(
        default_factory=lambda: {
            "left": FeetechMotorsBusConfig(
                port="/dev/aloha_follower_arm_left",
                motors={
                    # name: (index, model)
                    "shoulder_pan": [1, "sts3215"],
                    "shoulder_lift": [2, "sts3215"],
            ...
            "right": FeetechMotorsBusConfig(
                port="/dev/aloha_follower_arm_right",
                motors={
                    # name: (index, model)
                    "shoulder_pan": [1, "sts3215"],
                    "shoulder_lift": [2, "sts3215"],
